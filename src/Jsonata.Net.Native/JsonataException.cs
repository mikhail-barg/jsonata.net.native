...
namespace Jsonata.Net.Native
{
     ...
     * Nxxxx    - Non standard errors added in Jsonata.Net.Native implementation
     */
    public enum JsonataErrorCode
    {
        ...
        U1001,  /// probably - unbound execution

        N0001, /// Pattern argument of the {nameof(match)} function is not a regex
        N0002, /// Argument 2 of function {nameof(sort)} should be a function(left, right) returning boolean
        N0003, /// Argument 1 of regex should be String, got {arg.Type}
        N0004, /// Error evaluating function
    }


    public class JsonataException: Exception
    {
        public JsonataErrorCode Code { get; }
        public string RawMessage { get; }
        public int? Position { get; }

        private static string BuildMessage(JsonataErrorCode code, string message, int position)
        {
            if (position >= 0)
            {
                return $"{code}: {message} (at position {position})";
            }
            else
            {
                return $"{code}: {message}";
        ...
        public JsonataException(JsonataErrorCode code, string message, int position, Exception? innerException)
            : base(BuildMessage(code, message, position), innerException)
        {
            this.Code = code;
            this.RawMessage = message;
            this.Position = position >= 0? position : null;
        }

        public JsonataException(JsonataErrorCode code, string message)
            : this(code, message, -1, null)
        {
        }

        public JsonataException(JsonataErrorCode code, string message, int position)
            : this(code, message, position, null)
        {
        }

        protected JsonataException(JsonataErrorCode code, string message, bool noCodeInMessage)
            : base(message)
        {
            this.Code = code;
            this.RawMessage = message;
    ...
    public sealed class JsonataAssertFailedException: JsonataException
    {
        public JsonataAssertFailedException(string message)
            : base(JsonataErrorCode.D3141, message, noCodeInMessage: true)
        {
        }
    }
}
